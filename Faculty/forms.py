...
class SearchForm(forms.Form):
    ...
    FacultyCollegeID = forms.CharField(required=False, max_length=64, error_messages={
        'required': 'Please write your old password'}, widget=forms.TextInput({'class': 'form-control', 'placeholder': 'Employee ID'}))

class FacultyCSV(forms.Form):
    CSV = forms.FileField(required= True, label="Faculty Details CSV File", widget=forms.FileInput({'class': 'form-control', 'accept': 'text/csv'}))